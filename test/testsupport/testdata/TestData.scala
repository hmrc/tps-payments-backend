...
object TestData {
  ...
  def startJourneyRequestMibOrPngr(
    taxRegimeDisplay:    String,
    taxType:             TaxType,
    paymentSpecificData: PaymentSpecificData
  ): StartJourneyRequestMibOrPngr = startjourneymodel.StartJourneyRequestMibOrPngr(
    pid = "pid",
    payments = Seq[SjPaymentItem](
      SjPaymentItem(
        chargeReference = "chargeReference",
  ...
  val mibPayments: Journey          =
    startJourneyRequestMibOrPngr("MIB", TaxTypes.MIB, MibSpecificData("chargeReference", BigDecimal(1), BigDecimal(2)))
      .makeJourney(created)
  val childBenefitPayments: Journey = startJourneyRequestMibOrPngr(
    "ChildBenefitsRepayments",
    TaxTypes.ChildBenefitsRepayments,
    ChildBenefitSpecificData("childBenefitRef")
  ).makeJourney(created)
  val saPayments: Journey           =
    startJourneyRequestMibOrPngr("SA", TaxTypes.Sa, SaSpecificData("saRef")).makeJourney(created)
  val sdltPayments: Journey         =
    startJourneyRequestMibOrPngr("SDLT", TaxTypes.Sdlt, SdltSpecificData("sdltRef")).makeJourney(created)
  val safePayments: Journey         =
    startJourneyRequestMibOrPngr("SAFE", TaxTypes.Safe, SafeSpecificData("safeRef")).makeJourney(created)
  val cotaxPayments: Journey        =
    startJourneyRequestMibOrPngr("COTAX", TaxTypes.Cotax, CotaxSpecificData("cotaxRef")).makeJourney(created)
  val ntcPayments: Journey          =
    startJourneyRequestMibOrPngr("NTC", TaxTypes.Ntc, NtcSpecificData("ntcRef")).makeJourney(created)
  val payePayments: Journey         =
    startJourneyRequestMibOrPngr("PAYE", TaxTypes.Paye, PayeSpecificData("payeRef", BigDecimal(100), BigDecimal(100)))
      .makeJourney(created)
  val npsPayments: Journey          =
    startJourneyRequestMibOrPngr("NPS", TaxTypes.Nps, NpsSpecificData("npsRef", "1", "2", "3", BigDecimal(10)))
      .makeJourney(created)
  val vatPayments: Journey          =
    startJourneyRequestMibOrPngr("VAT", TaxTypes.Vat, VatSpecificData("vatRef", "someRemittanceType")).makeJourney(
      created
    )
  val pptPayments: Journey          =
    startJourneyRequestMibOrPngr("PPT", TaxTypes.Ppt, PptSpecificData("pptRef")).makeJourney(created)
  ...
  // language=JSON
  val chargeRefNotificationPciPalRequestJson: JsValue = Json.parse(s"""{
            "HoD": "B",
            "TaxReference": "$reference",
            "Amount": 1.92,
            "Commission": 1.23,
  ...
  // language=JSON
  val tpsPaymentsJson: JsValue = Json.parse(s"""{
          "_id" : "${id.value}",
  ...
  // language=JSON
  val journeyMongoJson: JsValue = Json.parse(s"""{
          "_id" : "${id.value}",
          "pid" : "$pid",
          "created": {
            "$$date": {
  ...
  // language=JSON
  val tpsPaymentsMongoLegacyJson: JsValue = Json.parse(s"""{
          "_id" : "${id.value}",
          "journeyState" : {
            "Landing" : { }
          },
  ...
  val tpsItemsForEmail: String =
    """[{"taxType":"Child Benefits repayments","amount":"1.92","transactionFee":"1.23","transactionNumber":"3123456701"}]"""
  ...
  val modsReconLookup: List[MibSpecificData] = List(
    MibSpecificData(chargeReference = modsRef, vat = modsVatAmount, customs = modsCustomsAmount)
  )

  val modsPaymentCallBackRequestWithAmendmentRef: ModsPaymentCallBackRequest    =
